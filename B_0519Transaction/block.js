...
const calculateHash = (
  ...
  CryptoJS.SHA256(
    (index + data + timestamp + previousHash + difficulty + nonce).toString()
  ).toString();
...
const isValidBlockchain = (receiveBlockchain) => {
  ...
  for (let i = 1; i < receiveBlockchain.length; i++) {
    if (
      isValidNewBlock(receiveBlockchain[i], receiveBlockchain[i - 1]) == false
    ) {
...
const replaceBlockchain = (receiveBlockchain) => {
  ...
  if (isValidBlockchain(receiveBlockchain)) {
    ...
    if (
      receiveBlockchain.length > blocks.length ||
